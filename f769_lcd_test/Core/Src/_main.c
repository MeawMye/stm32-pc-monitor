...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
//#define LCD_FB_START_ADDRESS  ((uint32_t)0xC0000000U)
#define LCD_W 800
#define LCD_H 480
...
static void MX_DSIHOST_DSI_Init(void)
{
  ...
  VidCfg.HorizontalLine = 800 + 2;  // 약간 여유
...
static void LCD_Init(void)
{
  ...
  if (HAL_DSI_Refresh(&hdsi) != HAL_OK)
  {
    Error_Handler();
  }
}
...
static void LCD_FillWhite(void)
{
  volatile uint32_t *fb = (volatile uint32_t *)LCD_FB_START_ADDRESS;
  int x;
  int y;
